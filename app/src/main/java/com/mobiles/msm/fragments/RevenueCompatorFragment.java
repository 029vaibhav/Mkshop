...
import com.mobiles.msm.adapters.RevenueCompartorAdapter;
        ...
        category.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (fromDate.getText().toString().equalsIgnoreCase("from") || toDate.getText().toString().equalsIgnoreCase("to")) {
                    MyApplication.toast(getActivity(), "please select date range");
                } else {
                    {
                        ...
                        builder.setSingleChoiceItems(aa1, 0, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (which == 0 || which == 2) {
                                    Client.INSTANCE.getpricecompator(MyApplication.AUTH, sFromdate, sToDate, whichString[which]).enqueue(new Callback<List<Sales>>() {
                                        @Override
                                    ...
                                    Client.INSTANCE.getPriceComparatorTech(MyApplication.AUTH, sFromdate, sToDate).enqueue(new Callback<List<Sales>>() {
                                        @Override
                                        public void onResponse(Call<List<Sales>> call, Response<List<Sales>> response) {
