...
public class NewProductFragment extends Fragment {
    ...
    private void validateMobile() {
        ...
        if (product.getBrand() == null || product.getBrand().length() == 0) {
            MkShop.toast(getActivity(), "Brand cannot be empty");
        } else if (product.getModel() == null || product.getModel().length() == 0) {
            MkShop.toast(getActivity(), "Model cannot be empty");
        } else if (product.getPrice() == 0) {
            MkShop.toast(getActivity(), "Price of a mobile cannot be 0");
        } else {
            progressDialog.show();
            product.setType(ProductType.Mobile);
            Call<Product> productCall = Client.INSTANCE.createProduct(MkShop.AUTH, product);
            productCall.enqueue(new Callback<Product>() {
                ...
                public void onResponse(Call<Product> call, Response<Product> response) {
                    ...
                    if (response.isSuccessful()) {
                        MkShop.toast(getActivity(), "Successfully registered a new Mobile");
                ...
                public void onFailure(Call<Product> call, Throwable t) {
                    ...
                    if (t.getMessage() != null) {
                        ...
                        MkShop.toast(getActivity(), "some unknown error occurred");
                    }
                }
            });
        }
    ...
    public void getDataFromWebsite() {
        ...
        if (gsmArenaLink != null && gsmArenaLink.contains("gsmarena.com")) {
            ...
            MkShop.toast(getActivity(), "Sorry we are not able to get data from given url");
        }
    }
}
